...
#' @param by_vegetationType (logical) get occurrences by vegetation type.
#' Default = FALSE
...
get_pam <- function(data, by_Biome = TRUE, by_State = TRUE,
  ...
  if (!is.logical(by_vegetationType)) {
    stop(paste0("Argument by_vegetationType must be logical, not ",
                class(by_vegetationType)))
  }
  ...
  if(!by_State & !by_Biome & !by_vegetationType){
    stop("At least one of the parameters by_State, by_Biome or by_vegetationType must be TRUE")
  }
  ...
  if (by_vegetationType) {
    columns <- c(columns, "vegetationType")
  }
