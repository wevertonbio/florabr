...
#'
#' @description
#' This function checks if you have the latest version of the Brazilian Flora data available in a specified directory.
#'
...
check_version <- function(data_dir) {
  ...
  if(length(dir_versions) == 0) {
    cat(paste0("You do not have any version of Flora do Brazil 2020 in this directory. The latest version is ", latest_version, ". Please, change the directory or run the function get_florabr() to download the latest version of Brazilian Flora 2020.", "\n"))
  }

  if(isTRUE(is_latest) & isFALSE(many_versions)) {
    cat(paste("You have the latest version of Brazilian Flora 2020 Data - Version",
              latest_version, "\n"))
  }
  ...
  if(isFALSE(is_latest) & isTRUE(many_versions)) {
    cat(paste0("You have the following versions of Brazilian Flora:\n",
              ...
              latest_version,
    "\nIf you want to download the latest version, run the function get_florabr() again"))
  }
}
