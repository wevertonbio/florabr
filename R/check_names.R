#' Check species names
#'
#' @description check_names checks if the species names are correct and searches for suggestions if the name is misspelled or not found in the Brazilian Flora 2020 database.
#'
...
#' @param max_distance (numeric) Maximum distance (as a fraction) allowed for searching suggestions when the name is misspelled. It can be any value between 0 and 1. The higher the value, the more suggestions are returned. For more details, see \code{\link[base:agrep]{agrep}}. Default = 0.1.
...
#' - Spelling: indicates if the species name is Correct (a perfect match with a species name in the Brazilian Flora 2020), Probably_incorrect (partial match), or Incorrect (no match with any species).
#' - Suggested name: If Spelling is Correct, it is the same as the input_name. If Spelling is Probably_correct, one or more suggested names are listed, found according to the maximum distance. If Spelling is Incorrect, the value is NA.
...
check_names <- function(data = NULL, species = NULL, max_distance = 0.1,
  ...
  if (!is.numeric(max_distance)) {
    stop(paste0("Argument max_distance must be numeric, not ", class(max_distance)))
  }
  ...
  #Calculate distance
  spp$Distance <- sapply(1:nrow(spp), function(i){
  ...
  spp_info <- merge(spp, d_info, by.x ="Suggested_name", by.y = "species",
                    all = TRUE, sort = FALSE)
  #Organize columns
  spp_info <- spp_info[, c("input_name", "Spelling", "Suggested_name", "Distance", "taxonomicStatus",
