...
vignette: >
  %\VignetteIndexEntry{Using florabr to flag erroneous records}
...
## Loading data
Before you begin, use the `load_florabr` function to load the data. For more detailed information on obtaining and loading the data, please refer to [1. Getting started with florabr](getting_started.Rmd)
...
As default, `florabr` uses a simplified polygon of federal States and biomes, with a reduced number of nodes represenint the boundaries of states and biomes (check `?terra::simplifyGeom`):
...
plot(states[states$abbrev_state == "PR"], main = "Simplified geometries",
     mar = NA)
```
...
br_biomes <- terra::subset(br_biomes, br_biomes$name_biome != "Sistema Costeiro")
```

The names of the biomes are in Portuguese. We need to translate Amazonia and Mata Atlantica to English to match with the names in Brazilian Flora database:
...
br_biomes$name_biome[which(br_biomes$name_biome == "Mata Atlântica")] <- "Atlantic_Forest"
```
...
spp_spt_2 <- get_spat_occ(data = bf, species = spp,
                       ...
                       state_column = "abbrev_state", #Column name with states acronymns
